...
func NewModel(opts Options) (*Model, error) {
	...
	if opts.Username != "" && len(messages) > 0 {
		latest := messages[len(messages)-1]
		// Initialize room watermark if not set
		if roomWatermark, _ := db.GetReadTo(opts.DB, opts.Username, ""); roomWatermark == nil {
			_ = db.SetReadTo(opts.DB, opts.Username, "", latest.ID, latest.TS)
		}
		// Initialize thread watermarks for subscribed threads that don't have one
		for _, thread := range threads {
			if threadWatermark, _ := db.GetReadTo(opts.DB, opts.Username, thread.GUID); threadWatermark == nil {
				// Get latest message in thread and set watermark
				threadMsgs, err := db.GetMessages(opts.DB, &types.MessageQueryOptions{
					Home:  &thread.GUID,
					Limit: 1,
				})
				if err == nil && len(threadMsgs) > 0 {
					last := threadMsgs[len(threadMsgs)-1]
					_ = db.SetReadTo(opts.DB, opts.Username, thread.GUID, last.ID, last.TS)
				}
			}
		}
	}
